...
class BusManager {
    ...
    onRoomAlive = async (_topic, _message, _packet) => {
        ...
        if (_message.peerId in this.peerHousekeep) {
            ...
            // we need to releave it from its misery and allow it to leave
            // gracefully..
    ...
    startServerSidePortScripts = async (_roomName, _roomId) => {
        if(this.switchBoardURI !== null){
            ...
            for (let i = 0; i < 20; i++) {
                // OSC proxies
                try{
                    await this.addSwitchBoardProxy(_roomName,_roomId * 1000 + i * 5, 'many2manyBi', "UDP proxy for OSC, channel:" + i);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of starting up many2manyBi script on port ${_roomId * 1000 + i * 5}.`);
                }
                // Ultragrid video and audio proxies
                try{
                    await this.addSwitchBoardProxy(_roomName, _roomId * 1000 + 100 + i * 5, 'one2manyMo', "Ultragrid video proxy, channel:" + i);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of starting up one2manyMo script on port ${_roomId * 1000 + 100 + i * 5}.`);
                }
                try{
                    await this.addSwitchBoardProxy(_roomName, _roomId * 1000 + 100 + i * 5 + 2, 'one2manyMo', "Ultragrid audio proxy, channel:" + i);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of starting up one2manyMo script on port ${_roomId * 1000 + 100 + i * 5 + 2}.`);
                }
                // NatNet2OSC and NatNetBridge proxies
                try{
                    await this.addSwitchBoardProxy(_roomName, _roomId * 1000 + 200 + i * 5, 'one2manyBi', "MoCap/NatNet-Ctrl proxy, channel:" + i);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of starting up one2manyBi script on port ${_roomId * 1000 + 200 + i * 5}.`);
                }
                try{
                    await this.addSwitchBoardProxy(_roomName, _roomId * 1000 + 200 + i * 5 + 2, 'one2manyMo', "NatNet-data proxy, channel:" + i);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of starting up one2manyMo script on port ${_roomId * 1000 + 200 + i * 5 + 2}.`);
                }
            }
    ...
    stopServerSidePortScripts = async (_roomName, _roomId) => {
        if(this.switchBoardURI !== null) {
            ...
            for (let i = 0; i < 20; i++) {
                try{
                    await this.deleteSwitchBoardProxy(_roomId * 1000 + i * 5);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of stopping many2manyBi script on port ${_roomId * 1000 + i * 5}.`);
                }
                try{
                    await this.deleteSwitchBoardProxy(_roomId * 1000 + 100 + i * 5);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of stopping one2manyMo script on port ${_roomId * 1000 + 100 + i * 5}.`);
                }
                try{
                    await this.deleteSwitchBoardProxy(_roomId * 1000 + 100 + i * 5 + 2);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of stopping one2manyMo script on port ${_roomId * 1000 + 100 + i * 5 + 2}.`);
                }
                try{
                    await this.deleteSwitchBoardProxy(_roomId * 1000 + 200 + i * 5);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of stopping one2manyBi script on port ${_roomId * 1000 + 200 + i * 5}.`);
                }
                try{
                    await this.deleteSwitchBoardProxy(_roomId * 1000 + 200 + i * 5 + 2);
                } catch(err){
                    console.error(`      ...: ${err}. Interrupting process of stopping one2manyMo script on port ${_roomId * 1000 + 200 + i * 5 + 2}.`);
                }
            }
