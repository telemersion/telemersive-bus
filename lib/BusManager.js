...
class BusManager {
    ...
    onRoomAlive = async (_topic, _message, _packet) => {
        ...
        if (this.peerHousekeep[_message.peerId] !== null) {
            // bevore we send back a room access message, we have to make sure the room alive ping
            ...
            if (this.rooms[_message.roomName].roomPwd === shaPassword) {
                ...
                await this.communicator.publish(
                    new BusMsgPub(
                        ...
                        peerId: _message.peerId,
                        peerName: _message.peerName,
                        roomId: this.rooms[_message.roomName].roomId,
                        roomName: _message.roomName,
                        roomUuid: this.rooms[_message.roomName].roomUUID,
                        reason: "confirmation",
                    }));
            }
        ...
        await this.communicator.publish(
            new BusMsgPub(
                ...
                RoomAccess, 2, false).encode({
                    ...
                    roomName: _message.roomName,
                    roomUuid: (accessGrant)?this.rooms[_message.roomName].roomUUID:"0",
                    reason: reason,
                }));
