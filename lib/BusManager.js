...
class BusManager {
    ...
    constructor(_baseTopic) {
        ...
        this.chatManagers = {};
        this.switchBoardURI = "http://localhost:3591/proxies/";
    }
    ...
    configureSwitchBoard = (_url, _port) => {
        this.switchBoardURI = _url + ":" + _port + "/proxies/";
    }
    ...
    startServerSidePortScripts = async (_roomName, _roomId) => {
        ...
        for (let i = 0; i < 10; i++) {
            await this.addSwitchBoardProxy(_roomId * 1000 + i, 'one2oneBi', "Room: " + _roomName + " -> UDP proxy, channel:" + i);
        }
        for (let i = 0; i < 10; i++) {
            await this.addSwitchBoardProxy(_roomId * 1000 + 100 + i * 10, 'one2oneBi', "Room: " + _roomName + " -> Ultragrid proxy, channel:" + i);
        }
     ...
     */
    addSwitchBoardProxy = async (_port, _type, _description) => {
        try {
            let payload = { port: _port, type: _type, description: _description };
            const res = await superagent.post(this.switchBoardURI).send(payload);
    ...
    deleteSwitchBoardProxy = async (_port) => {
        try {
            const res = await superagent.delete(this.switchBoardURI + _port);
            let reply = JSON.parse(res["text"]);
        } catch (err) {
            console.error(err);
        }
