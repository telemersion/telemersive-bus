...
class BusManager {
    ...
    startServerSidePortScripts = async (_roomName, _roomId) => {
        ...
        for (let i = 0; i < 10; i++) {
            await this.addSwitchBoardProxy(_roomName,_roomId * 1000 + i, 'one2oneBi', "UDP proxy, channel:" + i);
        }
        for (let i = 0; i < 10; i++) {
            await this.addSwitchBoardProxy(_roomName, _roomId * 1000 + 100 + i * 10, 'one2oneBi', "Ultragrid proxy, channel:" + i);
        }
